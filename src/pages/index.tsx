...
import { Box, Button, Card, Center, Code, FileInput, Grid, Kbd, MultiSelect, Stack, Switch, Text, Title } from '@mantine/core';
...
const shouldHideFromExcludes = (fileName: string, exclude: string[]) => {
...
const shouldHideFromOnlyChanges = (highlights: Record<number, unknown>, onlyChanged: boolean) => {
	return onlyChanged && Object.keys(highlights).length === 0;
};
					...
					<Stack>
						{showHead && (
							<>
								...
								<Stack spacing="sm" className="print-hide">
									...
									<Switch label="Show only changed files" {...form.getInputProps('onlyChanged')} />
						...
						{Object.entries(fileDict)
							...
							.map(([fileName, { content, hightlights, language }]) => (
								<MemoizedPreview
									key={fileName}
									hide={shouldHideFromExcludes(fileName, form.values.exclude) || shouldHideFromOnlyChanges(hightlights, form.values.onlyChanged)}
									fileName={fileName}
									language={language}
